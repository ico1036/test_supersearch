...
4. **🚨 핫플레이스 (필수 제외!)** - 아래 조건 하나라도 해당 시 메인 추천에서 제외:
   - 웨이팅 30분 이상 언급되는 곳
   - "줄 서는 맛집", "오픈런 필수" 키워드
   - "평일에도 웨이팅", "항상 붐비는" 곳
   - 전국 5대/10대 XX 타이틀 보유
   - TV 방송 출연 맛집 (생활의달인, 백종원, 수요미식회 등)
   - → 단, "시간 여유 시 옵션"으로 별도 섹션에 참고용 표기 가능
...
### Step 3.5: 🚨 핫플레이스 필터링 (필수!)

> **웨이팅 검색 결과를 분석하여 핫플레이스 제외 판정 실행**

```
자동 제외 조건 (하나라도 해당 시 메인 추천 제외):
  - "웨이팅 30분", "대기 1시간" 등 장시간 대기 언급
  - "줄 서는 맛집", "오픈런 필수" 키워드
  - "평일에도 웨이팅", "항상 붐비는" 키워드
  - "전국 5대", "전국 10대", "3대 맛집" 타이틀
  - "TV 출연", "방송 맛집", "생활의달인" 언급

제외 시 처리:
  1. 해당 장소 메인 추천에서 제외
  2. "⚠️ 핫플레이스 주의" 라벨 부착
  3. 대체 장소 추가 검색: "{location} {음식종류} 웨이팅없는 현지인"
  4. 대체 장소로 메인 추천 교체
  5. 제외된 장소는 "시간 여유 시 옵션"으로 별도 표기
```
...
□ 🚨 핫플레이스 필터링 완료 (Step 3.5)
   - 웨이팅 30분+ 장소 → 메인 추천에서 제외했는가?
   - "줄 서는 맛집" → 메인 추천에서 제외했는가?
   - TV 출연/전국 X대 맛집 → 메인 추천에서 제외했는가?
...
| 1.3.0 | 2025-01 | **핫플레이스 필터링 (Step 3.5) 추가** - 웨이팅 30분+, TV출연, 전국X대 맛집 자동 제외 로직 |
