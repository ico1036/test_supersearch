# Travel Planner Execution Guide v1.3
...
## Phase 3.5: 핫플레이스 제외 판정 (NEW - 필수!)

> **⚠️ 이 단계를 건너뛰면 안 됩니다!**
> Phase 3 검색 결과를 분석하여 "핫플레이스"를 반드시 걸러내야 합니다.

### 자동 제외 조건 (하나라도 해당 시 메인 추천에서 제외)

```javascript
// 핫플레이스 판정 키워드
const HOTPLACE_KEYWORDS = [
  // 웨이팅 관련
  "웨이팅 30분", "웨이팅 1시간", "대기 30분",
  "줄 서는 맛집", "오픈런 필수", "오픈런 추천",
  "평일에도 웨이팅", "평일에도 줄", "항상 붐비는",
  "주말 웨이팅 필수", "예약 필수",
...
FOR EACH 후보장소:
  IF 웨이팅 검색 결과에 HOTPLACE_KEYWORDS 포함:
    → hotplace_flag = true
    → 메인 추천 제외
    → "시간 여유 시 옵션"으로 분류
```
...
3. 대체 장소 추가 검색 실행:
   WebSearch({
     query: "{location} {음식종류} 웨이팅없는 현지인 숨은"
   })
4. 대체 장소로 메인 추천 교체
```
...
```markdown
❌ 제외 대상 (실제 사례):
- 원조강릉교동반점: "평일에도 60분 웨이팅" → 제외
- 초당순두부마을 유명점: "주말 1시간 대기" → 제외
- 안목커피거리 유명 카페: "줄 서는 카페" → 제외

✅ 통과 대상:
- 금성해장국: "오픈 직후 한산, 09시 이후 웨이팅" → 시간 조건부 통과
- 동네 주민 단골집: "웨이팅 없음" → 통과
```
...
|--------|-----------|-------------|---------|
| 교동반점 | 평일에도 웨이팅 60분 | 30-60분 | 오픈 10시 전 도착 필수 |
| XX식당 | TV 방송 출연 후 폭증 | 40분+ | 평일 14시 이후 권장 |
```
...
2. 🚨 핫플레이스 / 웨이팅 맛집 (Phase 3.5에서 필터링)
   - 웨이팅 30분 이상 → ❌ 메인 추천 제외
   - "줄 서는 맛집" → ❌ 메인 추천 제외
   - "전국 X대 맛집" → ❌ 메인 추천 제외
   - TV 방송 출연 맛집 → ❌ 메인 추천 제외
   - 평일에도 붐비는 곳 → ❌ 메인 추천 제외
   → 단, "시간 여유 시 옵션"으로 별도 섹션에 표기 가능
...
□ 🚨 Phase 3.5 핫플레이스 필터링 완료 ← 필수!
   - 웨이팅 30분+ 장소 메인 추천에서 제외했는가?
   - "줄 서는 맛집" 메인 추천에서 제외했는가?
   - TV 출연/전국 X대 맛집 제외했는가?
□ 5개 이상 대안 옵션 제시
□ 모든 장소에 "추천 이유" 구체적 명시
...
| 1.3.0 | 2025-01 | Phase 3.5 핫플레이스 필터링 추가 - 웨이팅 30분+/TV출연/전국X대 맛집 자동 제외 |
